...
least $n/k$ times.[^1] Note that there can be at most $k$ such values;
...
here are some high-level examples:[^2]
...
occur at least twice in the input array $A$.[^4] Suppose $A$ has the
form
...
1.  It has been implemented in real systems. For example, AT[&]{}T has
    ...
    using limited memory [@att].[^7] At Google, a precursor of the
...
premium for everything, even spellcheckers.[^8] This century, bloom
...
The count-min-sketch supports two operations: ($x$) and ($x$).[^9] The
...
think of $b$ as in the thousands and of $\ell$ as 5.[^10] The point
...
almost always behave like random data.[^11]
...
the hash functions $h_i$ are independent,[^12] we have
...
[*even if the random variables are not independent*]{}.[^13] The
...
$\tfrac{\eps n}{e}$.[^14] Since this overestimate is always nonnegative,
...
-   The space required is that for $\tfrac{e}{\eps} \ln
    ...
    scales linearly.[^15] In any case, the number of counters is totally
...
[^1]: A similar problem is the “top-$k$ problem,” where the goal is to
...
[^2]: You wouldn’t expect there to be a majority element in any of these
    applications, but you might expect a non-empty set of heavy hitters
    when $k$ is 100, 1000, or 10000.

[^3]: Rather than thinking of the array $A$ as an input fully specified
...
[^4]: A simple modification of this argument extends the impossibility
    result to all interesting values of $k$ — can you figure it out?

[^5]: Somewhat more detail: if you always use sublinear space to store
...
[^6]: This impossibility result (Fact \[fact:hh\]) and our response to
...
[^7]: There is a long tradition in the Internet of designing routers
    that are “fast and dumb,” and many of them have far less memory than
    a typical smartphone.

[^8]: The proposal was to insert all correctly spelled words into a
    bloom filter. A false positive is then a misspelled word that the
    spellchecker doesn’t catch.

[^9]: The same data structure supports weighted increments, of the form
...
[^10]: Where do we get $\ell$ hash functions from? The same way we get a
...
[^11]: In an implementation that chooses $h_i$ deterministically as a
...
[^12]: Don’t forget that probabilities factor only for independent
...
[^13]: Note the analogous statement for products is false if the $X_j$’s
...
[^14]: Here $e = 2.718\ldots$, which gives slightly better constant
    factors than the choice of 2 in Section \[ss:error\_heur\].

[^15]: Note that the challenging case, and the case that often occurs in
...
[^16]: How is this possible? Intuitively, with an error of $\eps n$
