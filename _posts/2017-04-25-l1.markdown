...
The solution \eqref{eq:simple} of mapping URLs to caches is an excellent first cut, and it works great in many cases.  To motivate why we might need a different solution, suppose the number $$n$$ of caches is not static, but rather is changing over time.  For example, in Akamai's early days, they were focused on adding as many caches as possible all over the Internet, so $$n$$ was constantly increasing.  Web caches can also fail or lose connection to the network, which causes $$n$$ to decrease.  In a peer-to-peer context (see the section [Some History](#some-history)), $$n$$ corresponds to the number of nodes of the network, which is constantly changing as nodes join and depart.
