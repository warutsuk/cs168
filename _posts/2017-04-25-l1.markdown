...
To understand which objects are assigned to which caches, consider the array shown in [Figure 2](#fig2), indexed by the possible hash
...
![array](/cs168/images/l1/array.jpg){:height="80%" width="80%" style="display:block;margin:auto;"}
{: #fig2}
...
This approach to consistent hashing can also be visualized on a circle, with points on the circle corresponding to the possible hash values ([Figure 3](#fig3)(left)).  Caches and objects both hash to points on this circle; an object is stored on the cache server that is closest in the clockwise direction.  Thus $$n$$ carches partition the circle into $$n$$ segments, with each cache responsible for all objects in one of these segments.

![circle](/cs168/images/l1/circle.jpg){:height="80%" width="80%" style="display:block;margin:auto;"}
{: #fig3}
