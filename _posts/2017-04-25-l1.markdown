...
This approach to consistent hashing can also be visualized on a
circle, with points on the circle corresponding to the possible hash
values (Figure \ref{f:circle}(left)).  Caches and objects both hash to
points on this circle; an object is stored on the cache server that is closest in the
clockwise direction.  Thus $$n$$ carches partition the circle into $$n$$
segments, with each cache responsible for all objects in one of these
segments.
