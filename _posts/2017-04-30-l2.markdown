...
definition of the $$\epsilonilon$$-approximate heavy hitters problem
([Section 1.4](#sec1-4)): the goal is to identify objects with
frequency count at least $$\tfrac{n}{k}$$, without being fooled by any
objects with frequency count less than $$\tfrac{n}{k} -\epsilonilon n$$.  This
means we just need to estimate the frequency count of an object up to
additive one-sided error $$\epsilonilon n$$.  If we take the number of buckets
...
$$
\prob{\min_{i=1}^{\ell} \text{CMS}[i][h_i(x)]
 > f_x + \frac{2n}{b}}
= \prod_{i=1}^{\ell} \prob{\text{CMS}[i][h_i(x)]
 > f_x + \frac{2n}{b}}
