...
1. It has been implemented in real systems.  For example, AT{\&}T has
  ...
  limited memory~\cite{att}.\footnote{There is a long tradition in the
    Internet
    of designing routers that are "fast and dumb," and many of them
    have far less memory than a typical smartphone.}
   At Google, a precursor of the count-min sketch (called the "count
  sketch"~\cite{CCF02}) has been implemented on top of their MapReduce
  parallel processing infrastructure~\cite{googlecms}.  One of the original
