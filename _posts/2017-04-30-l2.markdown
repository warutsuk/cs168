...
* Assuming that each hash function $$h_1,\ldots,h_{\ell}$$ is chosen
 ...
 the estimate Count($$x$$) output by the count-min sketch is greater than
 $$f_x + \epsilon n$$ is at most $$\delta$$. One would expect comparable performance for fixed well-crafted hash
